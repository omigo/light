...
// It is generated by `light` tool by source `model.go` at 2017-03-17 15:58:05.
...
// count: select count(*) from models where name like ${m.Name} [{ m.Flag != false } and flag=${m.Flag} ] [{ len(ss) != 0 } and status in ( [{range ss}] ) ]
func (*ModelMapperImpl) Count(tx *sql.Tx, m *model.Model, ss []enum.Status) (i int64, err error) {
	...
	log.Debug(query)
	log.Debug(args...)
	log.Error("TODO page template not implement")
	return
}

// list: select id, name, flag, score, map, time, xarray, slice, status, pointer, struct_slice, uint32 from models where name like ${m.Name} [{ m.Flag != false } [{ len(ss) != 0 } and status in ( [{range ss}] ) ] and flag=${m.Flag} ] [{ len(m.Array) != 0 } and xarray && array[ [{range m.Array}] ] ] [{ len(m.Slice) != 0 } and slice && array[ [{range m.Slice}] ] ] order by id offset ${offset} limit ${limit}
