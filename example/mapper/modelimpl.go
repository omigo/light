...
// It is generated by `light` tool by source `model.go` at 2017-03-21 19:30:10.
...
// page: select id, name, flag, score, map, time, slice, status, pointer, struct_slice from models where name like ${m.Name} [{ m.Flag != false } [{ len(ss) != 0 } and status in ( [{range ss}] ) ] and flag=${m.Flag} ] [{ len(m.Slice) != 0 } and slice && ${m.Slice} ] order by id offset ${offset} limit ${limit}
